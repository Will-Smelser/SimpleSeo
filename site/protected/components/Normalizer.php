...
class Normalizer {
        ...
        for($i=strlen($host)-1;$i >= 0; $i--){
            if($host[$i] == '.'){
                if($tld == null){
                    ...
                    $name = substr($host,$i+1);

                    //just easier than figuring out the actual offset
                    $name = str_replace('.'.$tld,'',$name);
                    break;
                }
            }
        }
        if($name == null && $tld !== null && strlen($host) > strlen($tld))
            $name = str_replace('.'.$tld,'',$host);

        if($tld !== null && $name !== null)
            return $name . '.' . $tld;
    ...
    public function setUrl( $url ) {
        //lets make sure there is a http on front
