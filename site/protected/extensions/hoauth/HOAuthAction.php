...
class HOAuthAction extends CAction
{
	...
	/**
	 * Custom attribute mapping of hybrid User_Profile class
	 * to yii-user profile fields.  All mapped fields will get
	 * set with atleast a value of null.
	 * 
	 * STRUCTURE:<br>
	 * <code>
	   array(
	    ...,
			array(
				'user-field'=>'String-field value in yii-user profiles table',
				'hybrid-field'=>'String-member variable of Hybrid_User_Profile class' | array('An array of string member varibales'),
				'join'=>'[optional] String-delimiter used for joining, if hybrid-field is an array'
				'valid-format'=>'[optional] String-regex expression to test hybrid result value against'		
			),
		...
	   </code>
	 * 
	...
	public $profiles=array(
		array(
			...
			'hybrid-field'=>'firstName',		
		),
		array(
			'user-field'=>'lastname',
			'hybrid-field'=>'lastName',
		),
		array(
			'user-field'=>'birthday',
			'hybrid-field'=>array('birthYear','birthMonth','birthDay'),
			'join'=>'-',
			'valid-format'=>'/[\d]{4}-[\d]{2}\-[\d]{2}/'
		),
	);
	
	/**
	 * Merge the Hybrid_User_Profile class data into the yii-user Profile class
	 * data using $this->profiles information.  If there are issues, turn on
	 * logging as some useful information may be presented.
	 * 
	 * @param object &$userProfile Reference to the Hybrid_User_Profile object
	 * @param object &$profile Reference to the yii-user Profile object
	 */
	private function mergeProfiles(&$userProfile, &$profile){
		$delimiterDefault = '-';
		
		//build the list of yii-user profile fields
		$pfields = array();
		foreach($profile->getFields() as $field)
			array_push($pfields,$temp = $field->getAttribute('varname'));
		
		$check = false;
		
		// go through the profile array, copying entries into $profiles
		foreach($this->profiles as $map){
			$pfield = $map['user-field'];
			$hfield = $map['hybrid-field'];
			
			//only add if yii-user profile has the field
			if(in_array($pfield, $pfields)){
				//initialize value
				$value = null;
				
				// if hybrid-field is an array, then we are mapping
				// multiple hybrid fields into a single yii-user profile
				// field.
				if(is_array($hfield)){
					$del = isset($map['join']) ? $map['join'] : $delimiterDefault;
					$temp = array();
					
					// cycle the hybrid-fields getting their values
					// and join their values with $del (the delimiter)
					foreach($hfield as $f){
						if(!isset($userProfile->$f))
							break;
						array_push($temp,$userProfile->$f);
					}
					$value = implode($del,$temp);

				// 1-1 mapping, just ensure hybrid has the falue
				}elseif(isset($userProfile->$hfield)){
					$value = $userProfile->$hfield;
				}
				
				// check value against regular expression, reset to null if it fails
				if(isset($map['valid-format']) && !preg_match($map['valid-format'],$value)){
					Yii::log("WARNING: Failed yii-user profile value ($pfield : $value).  Regex check failed ({$map['valid-format']})",
						CLogger::LEVEL_INFO, 'hoauth.'.__CLASS__);
					$value = null;
						
				}
				
				$temp = ($value === null) ? 'NULL' : $value;
				Yii::log("setting yii-user profile value ($pfield : $temp)", CLogger::LEVEL_INFO, 'hoauth.'.__CLASS__);
				
				$profile->setAttribute($pfield,$value);
				
			}else{
				Yii::log("WANRING: Skipped yii-user profile field ($pfield)", CLogger::LEVEL_INFO, 'hoauth.'.__CLASS__);
			}	
		}
	}
	...
	*/
	...
	protected function processUser($user, $userProfile)
	{
		...
		if($user->isNewRecord)
		{
			...
			//requires we have a password, so lets make one
			$length = 10;
			$chars = array_merge(range(0,9), range('a','z'), range('A','Z'));
			shuffle($chars);
			$user->password = implode(array_slice($chars, 0, $length));
			$user->status = 1;
